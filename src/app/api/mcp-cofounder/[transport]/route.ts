...
import { z } from "zod";
import { supabase, writeAdminLog } from "@/lib/mcp-shared";
...
const DEFAULT_STATE = {
  ...
  approvalBoundaries: ["spending money", "external communications", "public publishing", "production changes", "financial transactions", "payment configuration", "data deletion", "legal commitments", "security controls"],
  activeGoal: "Launch Vuka from zero traction using controlled acquisition experiments and convert the first cohort into repeat users, artists and sales.",
  updatedAt: isoNow(),
};
...
const DEFAULT_CAMPAIGN_LAB = {
  ...
  status: "planning",
  tracks: {
    artists: { label: "Artist acquisition + activation", allocationZAR: 1800, objective: "Recruit artists, get music live, and create conditions for first genuine fan purchases." },
...
const DEFAULT_FOUNDING15 = {
  ...
  budgetZAR: 3000,
  maxParticipants: 15,
  rewards: { signupUploadAndFirstPurchase: 100, crowdfunding3Backers: 300, albumOr8BeatsAndPurchase: 200 },
...
const server = createMcpHandler((mcp) => {
  mcp.tool("get_cofounder_state", "Read the durable operating state for the ChatGPT/Vuka AI co-founder relationship.", {}, async () => {
    const [state, goals, decisions, approvals, actions] = await Promise.all([
      getCofounderSetting("cofounder.state", DEFAULT_STATE), getCofounderSetting("cofounder.goals", []), getCofounderSetting("cofounder.decisions", []), getCofounderSetting("cofounder.approvals", []), getCofounderSetting("cofounder.actions", [])
    ]);
  ...
  mcp.tool("propose_cofounder_decision", "Record a decision that needs human approval before a bounded execution step.", {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ ...decision, approvalRequired: true }, null, 2) }] };
  });
  ...
  mcp.tool("approve_cofounder_decision", "Record explicit founder approval for a previously proposed decision. This does not itself execute the action.", {
    ...
    await setCofounderSetting("cofounder.decisions", decisions.map((d) => d.id === decisionId ? { ...d, status: "approved", approvedAt: approval.approvedAt, approvalId: approval.id } : d));
  ...
  mcp.tool("record_cofounder_action", "Audit an action taken under an approved objective. Use approvalId for externally consequential actions.", {
  ...
  mcp.tool("initialize_campaign_lab", "Create the flexible R3,000 traction experiment with separate artist and friends/fan transaction tracks. Total allocation cannot exceed the hard ceiling.", {
    ...
    if (existing?.events?.length || existing?.commitments?.length) return { content: [{ type: "text", text: "Campaign lab already has activity. Refusing to reset it; update the allocation instead." }], isError: true };
  ...
  mcp.tool("get_campaign_lab_status", "Return the current artist/friends experiment split, commitments, transaction activity, and budget safety.", {}, async () => {
    ...
    const byTrack = ["artists", "friends"].reduce((a: any, track) => {
      const trackEvents = events.filter((e: any) => e.track === track), trackCommitments = commitments.filter((c: any) => c.track === track);
      a[track] = { allocationZAR: lab.tracks?.[track]?.allocationZAR ?? 0, committedZAR: trackCommitments.reduce((s: number, c: any) => s + (c.amountZAR ?? 0), 0), events: trackEvents.length, transactions: trackEvents.filter((e: any) => e.type === "genuine_transaction").length, transactionValueZAR: trackEvents.filter((e: any) => e.type === "genuine_transaction").reduce((s: number, e: any) => s + (e.valueZAR ?? 0), 0) };
      return a;
    }, {});
  ...
  mcp.tool("allocate_campaign_lab_budget", "Rebalance artist and friends/fan budgets. Existing commitments are protected and the R3,000 ceiling cannot be exceeded.", {
  ...
  mcp.tool("commit_campaign_lab_reward", "Reserve a campaign incentive against a specific track without sending money. Both track and global budget guards apply.", {
    ...
    const lab = await getCofounderSetting<any>("cofounder.campaign.lab", DEFAULT_CAMPAIGN_LAB);
    const commitments = lab.commitments ?? [], committed = commitments.reduce((s: number, c: any) => s + (c.amountZAR ?? 0), 0), trackCommitted = commitments.filter((c: any) => c.track === track).reduce((s: number, c: any) => s + (c.amountZAR ?? 0), 0), allocation = lab.tracks?.[track]?.allocationZAR ?? 0;
    ...
    return { content: [{ type: "text", text: JSON.stringify({ commitment, committedZAR: committed + amountZAR, remainingZAR: lab.budgetZAR - committed - amountZAR, approvalRequired: true }, null, 2) }] };
  });

  mcp.tool("record_campaign_lab_event", "Record a measurable artist or friends/fan campaign event. Genuine transactions require positive value and evidence; recording never creates a payment.", {
  ...
  mcp.tool("get_founding15_status", "Calculate Founding 15 participation, reward commitments, remaining budget, and conversion progress.", {}, async () => {
    ...
    const candidates = campaign.candidates ?? [], events = campaign.events ?? [], committed = candidates.reduce((s: number, c: any) => s + (c.rewardCommittedZAR ?? 0), 0);
  ...
  mcp.tool("update_founding15_candidate", "Add or update one Founding 15 candidate with budget protection.", {
    ...
    const rewardCommittedZAR = (input.signupUploadFirstPurchase ? campaign.rewards.signupUploadAndFirstPurchase : 0) + (input.crowdfunding3Backers ? campaign.rewards.crowdfunding3Backers : 0) + (input.albumOr8BeatsAndPurchase ? campaign.rewards.albumOr8BeatsAndPurchase : 0), old = candidates.find((c: any) => c.id === id);
    const newTotal = candidates.reduce((s: number, c: any) => s + (c.id === id ? rewardCommittedZAR : (c.rewardCommittedZAR ?? 0)), 0);
    ...
    await setCofounderSetting("cofounder.campaign.founding15", { ...campaign, candidates: old ? candidates.map((c: any) => c.id === id ? candidate : c) : [candidate, ...candidates], status: "active", updatedAt: isoNow() });
    await writeAdminLog("cofounder.founding15_candidate_updated", "CampaignCandidate", id, JSON.stringify(candidate));
  ...
  mcp.tool("record_founding15_event", "Record a verifiable Founding 15 milestone; never pays rewards.", { candidateId: z.string(), type: z.enum(["contacted", "signed_up", "music_uploaded", "first_purchase_confirmed", "crowdfunding_3_backers", "album_or_8_beats_and_purchase", "reward_approved", "reward_paid"]), evidence: z.string().min(1).max(2000), valueZAR: z.number().min(0).optional() }, async (input) => {
    const campaign = await getCofounderSetting<any>("cofounder.campaign.founding15", DEFAULT_FOUNDING15);
    if (!(campaign.candidates ?? []).some((c: any) => c.id === input.candidateId)) return { content: [{ type: "text", text: `Candidate ${input.candidateId} not found.` }], isError: true };
    const event = { id: crypto.randomUUID(), ...input, recordedAt: isoNow() };
  ...
  mcp.tool("analyze_founding15_budget", "Stress-test the R3,000 Founding 15 reward design before money is committed.", { participants: z.number().int().min(0).max(100).default(15), crowdfundingQualified: z.number().int().min(0).max(100).default(0), albumQualified: z.number().int().min(0).max(100).default(0) }, async ({ participants, crowdfundingQualified, albumQualified }) => {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ budgetZAR: campaign.budgetZAR, scenario: { participants, crowdfundingQualified, albumQualified }, maximumPossibleUnderScenarioZAR: total, remainingZAR: campaign.budgetZAR - total, withinBudget: total <= campaign.budgetZAR }, null, 2) }] };
  });
  ...
  mcp.tool("log_cofounder_research", "Store a research finding, source, confidence, and the decision it may affect.", { topic: z.string().min(2).max(300), finding: z.string().min(2).max(5000), source: z.string().max(1000).optional(), confidence: z.enum(["high", "medium", "low"]), implication: z.string().max(2000).optional() }, async (input) => {
    ...
    await appendCofounderSettingArray("cofounder.research", finding, 1000); await writeAdminLog("cofounder.research_logged", "ResearchFinding", finding.id, JSON.stringify(finding));
    return { content: [{ type: "text", text: JSON.stringify(finding, null, 2) }] };
  });

  mcp.tool("get_cofounder_decision_queue", "Return unresolved decisions and their approvals.", {}, async () => {
  ...
  mcp.tool("get_company_dashboard", "One-call founder dashboard combining live Vuka metrics, revenue, campaign state and open decisions.", {}, async () => {
